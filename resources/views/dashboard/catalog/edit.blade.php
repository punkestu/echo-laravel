...
@section('content')
    <section class="flex justify-between items-start flex-wrap gap-2 mb-2">
        <div>
            <h3 class="text-xl font-bold">Ubah Katalog</h3>
                ...
                <div id="item-list" class="flex gap-2">
                    ...
                    @else
                        @foreach ($catalog->catalogItems as $citem)
                            <div class="text-sm bg-blue-400 text-white flex items-center gap-2 mt-2 px-3 py-1 rounded-full">
                                <span>{{ $citem->item->name }} | Rp.
                                    {{ number_format($citem->item->price, 0, '.', ',') }}</span> X
                                <input type="hidden" name="items[{{ $loop->index }}][id]"
                                    value="{{ $citem->item->id }}">
                                <input type="number" name="items[{{ $loop->index }}][qty]" value="{{ $citem->qty }}"
                                    class="bg-white text-black px-2 py-1 w-14 rounded-md">
                                <button type="button" class="text-red-500" onclick="this.parentElement.remove()">
                                    <svg class="w-6 h-6 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                        width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
