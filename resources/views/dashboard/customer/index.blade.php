...
@section('content')
    <section class="flex justify-between items-start flex-wrap gap-2 mb-2">
        <div>
            <h3 class="text-xl font-bold">Pelanggan</h3>
            <p class="text-sm text-justify hidden md:block">
                Master data pelanggan yang digunakan untuk melakukan telemarketing.
            </p>
        </div>
        <a href="{{ route('dashboard.customer.create') }}" class="text-sm bg-light text-gray-800 px-3 py-1 rounded-md">
    ...
    <section class="overflow-auto w-full max-h-[62vh]">
        <table class="table-auto overflow-scroll w-[150vw] md:w-[100vw]">
            <thead>
                <tr>
                    ...
                    <th class="border-b py-1 px-2 w-10 text-left">Nama</th>
                    <th class="border-b py-1 px-2 w-10 text-left">No HP</th>
                    ...
                    <tr>
                        ...
                        <td class="px-2 pt-1 pb-2 align-top">
                            <a href="https://wa.me/{{ $item->nohp_withcode() }}" target="_blank"
                                class="underline text-blue-500">{{ $item->phone }}</a>
                        </td>
                        <td class="px-2 pt-1 pb-2 align-top">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('dashboard.customer.edit', $item->id) }}"
                                ...
                                <button
                                    onclick="toggleModal('delete-item-modal', setDeleteUrl('{{ route('dashboard.customer.delete', $item->id) }}'))"
                                    class="bg-red-500 text-white rounded-sm px-3 py-1"><svg
        ...
        @if ($customers->isEmpty())
            <div class="text-sm flex flex-col justify-center items-center gap-2 p-2">
                Tidak ada data
                <a href="{{ route('dashboard.customer.create') }}" class="text-blue-500 underline">Tambah Item Baru</a>
            </div>
        @endif
    </section>
