...
@section('content')
    <section class="flex justify-between items-start flex-wrap gap-2 mb-2">
        <div>
            <h3 class="text-xl font-bold">Ubah Pelanggan</h3>
            <p class="text-sm text-justify hidden md:block">
                Ubah pelanggan di dalam sistem. Pastikan untuk mengisi semua informasi yang diperlukan dengan
                benar.
            </p>
        </div>
        ...
        <form action="{{ route('dashboard.customer.update', $customer->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" required
                    ...
                    placeholder="Masukkan nama item" value="{{ old('name') ?? $customer->name }}">
                @if ($errors->has('name'))
            ...
            <div class="mb-4">
                ...
                <input type="text" name="phone" id="phone" required
                    class="px-2 py-1 mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Masukkan nama item" value="{{ old('phone') ?? $customer->phone }}">
                @if ($errors->has('phone'))
