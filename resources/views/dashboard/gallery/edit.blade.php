...
@section('content')
    <section class="flex justify-between items-start flex-wrap gap-2 mb-2">
        <div>
            <h3 class="text-xl font-bold">Ubah Gambar</h3>
            <p class="text-sm text-justify hidden md:block">
                Ubah gambar di dalam sistem. Pastikan untuk mengisi semua informasi yang diperlukan dengan
                benar.
            </p>
        </div>
        ...
        <form action="{{ route('dashboard.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" required
                    ...
                    placeholder="Masukkan judul ..." value="{{ old('title') ?? $gallery->title }}">
                @if ($errors->has('title'))
            ...
            <div class="mb-4">
                ...
                <input type="number" name="priority" id="priority" required
                    class="px-2 py-1 mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Masukkan prioritas ..." value="{{ old('priority') ?? $gallery->priority }}">
